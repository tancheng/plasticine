...
object Parser {
  def apply(path: String) = {
    val m = parseRaw(path)
    throw new Exception("Parsing PISA from JSON has been temporarily suspended!")
//    buildFromParsedJSON(m)
  }
  ...
  def parseRaw(path: String) = {
    ...
    throw new Exception("JSON Parsing temporarily suspended!")
    null
//    val result = JSON.parseFull(contents)
//    result match {
//      case Some(m) => m
//      case None    => throw new RuntimeException("Couldn't parse PISA file:\n" + file.toString)
//    }
...
//                                .map { parseValue(_) }
//
//    val udcInitAtConfig: List[Int] = Parser.getFieldList(m, "udcInitAtConfig")
//                                .asInstanceOf[List[String]]
//                                .map { parseValue(_) }
//
...
//                                .map { parseValue(_) }
//
//    val fifoMux: List[Int] = Parser.getFieldList(m, "fifoMux")
//                                .asInstanceOf[List[String]]
//                                .map { parseValue(_) }
//    CUControlBoxConfig(tokenOutLUT, enableLUT, tokenDownLUT, udcInit, udcInitAtConfig, decXbar, incXbar, tokenInXbar, doneXbar, enableMux, syncTokenMux, tokenOutXbar, fifoAndTree, tokenInAndTree, fifoMux)
//  }
//
...
//        }
//
//        val wswap = Parser.getFieldString(m, "wswap") match {
//          case "x" => 0
//          case n@_ => n.drop(1).toInt + 1
//        }
//
//        val rswap = Parser.getFieldString(m, "rswap") match {
//          case "x" => 0
//          case n@_ => n.drop(1).toInt + 1
//        }
//
