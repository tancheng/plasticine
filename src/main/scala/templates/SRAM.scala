...
class SRAMByteEnable(val w: Int, val n: Int, val d: Int) extends Module {
  ...
  when (io.wen) {
    ...
    val tmpVec = Vec.tabulate(n) { i => tmp(i*8+8-1, i*8) }
    val newVec = Vec.tabulate(n) { i => io.wdata(i*8+8-1, i*8) }
    val wdata = Vec.tabulate(n) { i =>
      val byteEn = io.mask(i)
      Mux(byteEn, newVec(i), tmpVec(i))
    }.reverse.reduce { Cat(_,_) }
    */
    mem.write(io.waddr, io.wdata, io.mask)
  }
