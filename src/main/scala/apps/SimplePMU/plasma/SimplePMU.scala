...
trait SimplePMUTrait {
  ...
  pmuBits.scalarInXbar.outSelect(0) = 3
  pmuBits.scalarOutXbar.outSelect(0) = 4
  pmuBits.scratchpad.stride = 1
  pmuBits.scratchpad.numBufs = 2
  pmuBits.wdataSelect = 3
  pmuBits.rdataEnable = List(1, 0, 1, 0)
  pmuBits.waddrSelect = SVT(CounterSrc, 3)
  pmuBits.raddrSelect = SVT(CounterSrc, 5)
