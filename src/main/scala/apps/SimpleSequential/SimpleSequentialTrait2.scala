...
trait SimpleSequentialTrait2 extends SimpleSequentialTrait1 {
  ...
  def config2:Unit = {
    ...
    cus(1)(1).asPCUBits.stages(0).opcode = Bypass
    ...
    cus(1)(1).asPCUBits.stages(3).opA = SVT(PrevStageSrc, 0)
    cus(1)(1).asPCUBits.stages(3).opB = SVT(PrevStageSrc, 0)
    ...
    cus(1)(1).asPCUBits.stages(4).opA = SVT(PrevStageSrc, 0)
    cus(1)(1).asPCUBits.stages(4).opB = SVT(PrevStageSrc, 0)
    ...
    cus(1)(1).asPCUBits.stages(5).opB = SVT(PrevStageSrc, 0)
    ...
    cus(1)(1).asPCUBits.stages(6).opA = SVT(PrevStageSrc, 0)
    cus(1)(1).asPCUBits.stages(6).opB = SVT(PrevStageSrc, 0)
    ...
    cus(1)(1).asPCUBits.stages(8).opcode = Bypass
    cus(1)(1).asPCUBits.stages(8).res = List(SVT(CurrStageDst, 8))
    cus(1)(1).asPCUBits.stages(8).fwd(8) = SVT(ALUSrc, 8)
    // Configuring lcus(0)(2) <- SeqCU7_x358
    lcus(0)(2).counterChain.chain = List(0,0,0,0,0)
    // SeqCU7_x358.udcounters=[PipeCU96_x357 -> TokBuf326,Top1_Top -> TokBuf312]
    // lcus(0)(2).udcs=[Some(TokBuf326),Some(TokBuf312),None,None]
    lcus(0)(2).control.childrenAndTree = List(1, 0, 0, 0)
    lcus(0)(2).control.siblingAndTree = List(0, 1, 0, 0)
    lcus(0)(2).control.incrementXbar.outSelect(0) = 7
    lcus(0)(2).control.incrementXbar.outSelect(1) = 6
    lcus(0)(2).control.udcDecSelect=List(1,1,-1,-1)
    lcus(0)(2).control.tokenOutXbar.outSelect(2) = 0
    lcus(0)(2).control.tokenOutXbar.outSelect(3) = 1
    lcus(0)(2).control.doneXbar.outSelect(0) = 0
    lcus(0)(2).control.pulserMax=2
