...
trait InOutArgTrait1 {
  ...
  def config1:Unit = {
    ...
    cus(0)(0).stages(0) = PipeStageBits(SVT(ScalarFIFOSrc, 0),SVT(ConstSrc, 4),SVT(), FixAdd, List(SVT(CurrStageDst, 0),SVT(CurrStageDst, 8)), Array.tabulate(16) { i => SVT() }, SVT(ConstSrc, 1))
