...
import plasticine.spade.ControlSwitchParams
...
//    val app = Vec(numStreams, new MemoryStream(w, v))
    ...
    val config = Input(new MAGConfig())
  })
  ...
  val commandFifoConfig = Wire(new FIFOConfig(d, 1))
...
//  sgPulse := sgPulser.io.out
  ...
  // Gather crossbar
  val switchParams = ControlSwitchParams(v, v)
  ...
  val wdata = Vec(List.tabulate(v) { i =>
    if (i == 0) {
      val mux = Module(new MuxN(UInt(w.W), v))
  ...
  if (rdataFifos.length > 0) {
    val readyMux = Module(new MuxN(Bool(), rdataFifos.length))
  ...
  if (wrespFifos.length > 0) {
    val readyMux = Module(new MuxN(Bool(), wrespFifos.length))
