...
class Sequential6(val numInputs: Int) extends Module {
  val io = new Bundle {
...
class Sequential6Tests(c: Sequential6) extends PlasticineTester(c) {
  val numIter = 5
...
object Sequential6Test {

  def main(args: Array[String]): Unit = {
    val (appArgs, chiselArgs) = args.splitAt(args.indexOf("end"))

    val numInputs = 5
    chiselMainTest(chiselArgs, () => Module(new Sequential6(numInputs))) {
      c => new Sequential6Tests(c)
    }
  }
}
