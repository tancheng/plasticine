...
//      burstSizeBytes, startDelayWidth, endDelayWidth, numCounters, inst))
  ...
  val t1 = Module(new TileStore(w, v))
...
class TileStorerTests(c: TileStorer,
  val rows: Int,
  val size: Int,
  val addr: Int,
  val colsize: Int
  ...
  def peekDram() {
    ...
    val issuedVldIn = peek(c.mc.mus(0).io.dram.vldIn)
    ...
    if (issuedVldIn > 0) {
      println("io.dram.vldIn is signaled")
      counts += 1
    }
  ...
  poke(c.mc.io.interconnects(0).dataVldIn, 1)
...
//    cycles += 1
//  }
  while ((cycles < 30000) & (counts != rows * size / 64)) {
    step(1)
    peekDram
    done = peek(c.io.done)
    cycles += 1
...
object TileStorerTest {

  def main(args: Array[String]): Unit = {
    val (appArgs, chiselArgs) = args.splitAt(args.indexOf("end"))

    if (appArgs.size != 2) {
      println(s"Usage: <TileStorerTest> <#rows> <row_size>")
      sys.exit(-1)
    }
    // Test params
    ...
    chiselMainTest(chiselArgs, () => Module(new TileStorer(w, d, v, numOutstandingBursts, burstSizeBytes, startDelayWidth, endDelayWidth, numCounters, config) )) {
      c => new TileStorerTests(c, rows, size, addr, colsize)
    }
  }
}
