...
class TileStorerTests(c: TileStorer,
  ...
  def peekDram() {
    ...
    val completionMaskIn = c.mc.mus(0).completionMask.foreach { s => peek(s.io.data.in) }
    val completionMaskOut = c.mc.mus(0).completionMask.foreach { s => peek(s.io.data.out) }
