...
class Sequential5(val numInputs: Int) extends Module {
  val io = new Bundle {
...
class Sequential5Tests(c: Sequential5) extends PlasticineTester(c) {
  val numIter = 5
...
object Sequential5Test {

  def main(args: Array[String]): Unit = {
    val (appArgs, chiselArgs) = args.splitAt(args.indexOf("end"))

    val numInputs = 5
    chiselMainTest(chiselArgs, () => Module(new Sequential5(numInputs))) {
      c => new Sequential5Tests(c)
    }
  }
}
