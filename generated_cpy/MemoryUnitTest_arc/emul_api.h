...
public:
  emul_api_t(mod_t* m) {
	...
	// adding DRAMSim logic
  	...
  	mcMem = getMemorySystemInstance("ini/DDR2_micron_16M_8b_x8_sg3E.ini", "system.ini", "..", "plasticineController", 16384);
...
private:
  ...
  {
  	MemoryUnit_t *pctrl = (MemoryUnit_t *)module;
    pctrl->MemoryUnit__io_dram_vldIn.values[0] = 1;
  	printf("[Callback] read complete: %d 0x%lx cycle=%lu\n", id, address, clock_cycl;
  }
  ...
  virtual inline void step() {
    ...
    if (pctrl->MemoryUnit__io_dram_vldOut.values[0] > 1)
    {
