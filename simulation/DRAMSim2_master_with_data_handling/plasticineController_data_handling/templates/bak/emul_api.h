...
private:
  ...
  virtual inline void step() {
	cout << ">>>>>>>>>> TX_COMP = " << ((PlasticineControllerTop_t *)module)->PlasticineControllerTop_dramsim__TX_COMP.values[0] << endl;
    module->clock(LIT<1>(0));
    // FIXME: should call twice to get the output for now
	steps = steps + 1;
	cout << "call clock_lo from step, steps = " << steps << endl;
    module->clock_lo(LIT<1>(0), false);
	if (steps == 7)
	{
		cout << ">>>>>>>>> step == 6, turning on TX_COMP" << endl;
		((PlasticineControllerTop_t *)module)->PlasticineControllerTop_dramsim__TX_COMP.values[0] = 1;
	}
  }
