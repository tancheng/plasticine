...
void init_tech_params(double technology, bool is_tag)
{
  ...
  for (iter=0; iter<=1; ++iter)
  {
    ...
    g_tp.wire_local.pitch    += curr_alpha * wire_pitch[g_ip.ic_proj_type][(ram_cell_tech_type == 4)?3:0];
    g_tp.wire_local.R_per_um += curr_alpha * wire_r_per_micron[g_ip.ic_proj_type][(ram_cell_tech_type == 4)?3:0];
    g_tp.wire_local.C_per_um += curr_alpha * wire_c_per_micron[g_ip.ic_proj_type][(ram_cell_tech_type == 4)?3:0];
